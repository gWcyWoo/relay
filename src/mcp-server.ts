  ...
  const server = new McpServer(
    { name: "relay", version: "0.4.0" },
    {
      ...
      instructions:
        ...
        "You are connected to Relay. Call register(sessionId, role) once; the result carries this session's token, which the user hands to a counterpart session. To bind to a counterpart, call register again with that session's token: each token adds one binding, your own token never changes, the counterpart may be on another machine, and it is told who bound to it (the result says whether that notice was delivered). Roles are unique among the sessions bound to one counterpart, so a binding whose role that counterpart already has is refused: pick another role and register again. The result's bound list is the only place your bindings are shown, so read it. Then send(message, selfSessionId, role, urgent) reaches a bound session by its role (e.g. 'review'); target (its provider, or a bound sessionId such as meta.from of a received push) is only needed when the role is unknown. Set urgent only when the message must interrupt the counterpart's current work (a Codex turn is steered instead of the message waiting behind it). A message pushed to you via the channel carries meta.from (sender sessionId), meta.provider and meta.role; answer with send using role=meta.role. Nothing relayed may stay hidden from the user: a pushed message is not rendered by the client, so start your response by showing who sent it and its full text (what you send is already visible in the send call itself, so do not repeat it). Every relayed message costs a full model turn on each side, so send fewer and shorter ones: the send result is the proof of delivery (pushed, or submitted to the counterpart), so never ask a counterpart to acknowledge, and never send connectivity checks; keep a message to the conclusion, file paths and acceptance criteria, about 20 lines at most; details already live in the code, tests, commits and files the message points to, so do not restate them. Sessions without a push channel (e.g. codex) block in send until the reply arrives, so keep waiting on that call, but only block when you asked a question: for a report, a result or a status update pass wait=false and send returns as soon as it is delivered (an answer, if any, arrives later as a new turn); after 4 minutes without a reply the call returns a note and the reply, when it comes, arrives as a new turn in your session, as does an answer that comes while you are not waiting or after you ended the turn that called send. Sending again while a reply is pending is fine: the newer send takes over the wait. Sessions with a push channel (claude) return immediately. Claude Code sessions see one reconnect right after register: Relay forces it so the session starts accepting channel pushes, and the registration survives it, so do not register again or reconnect manually. Call unregister to leave; bindings on both sides are dropped.",
    },
  );
    ...
    "send",
    {
      ...
      inputSchema: {
        ...
        wait: z
          .boolean()
          .default(true)
          .describe("Codex only (Claude always returns at once). true: block until the counterpart answers, for questions. false: return as soon as the message is delivered, for reports, results and status updates; an answer, if any, arrives later as a new turn."),
      },
    },
    async ({ message, selfSessionId, target, role, urgent, wait }) => {
      const reply = await relay.send({ message, selfSessionId, target, role, urgent, wait });
